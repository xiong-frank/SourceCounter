...
namespace sc
{
    ...
    public:
        ...
        auto GetFileCount() const { return m_FileQueue.size(); }

        // 从指定目录中提取指定后缀名的源文件到队列中
        unsigned int ExtractFile(const std::string& fromPath, const list_type& suffixes);

        unsigned int Load(const std::string& input, const std::vector<std::string>& langs, const std::string& excludes) { return 0; }

        // 启动线程
        bool Start(unsigned int nThread, ReportList& reports, const list_type& singles, const pair_list& multiples);
        bool Start(unsigned int nThread) { return false; }

        bool Output(const std::string& output, unsigned int detail) const { return false; }
    };

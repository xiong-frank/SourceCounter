...
* Dependent third-party libraries. These libraries are single header file dependencies and have been copied into our project:
  ...
  | [FrankXiong/SimpleTest](https://github.com/xf-bnb/SimpleTest) | 1.0.1 | Simple test framework |
...
* In the [./projects](./projects) directory, the supported build project configurations or scripts are provided for each platform and compiler. Open the configuration directory of the corresponding environment and execute the corresponding build script or run the build. The successful build will be output to `./outputs` directory:
  * **Windows:** requires `VS 2017 15.7` or above.
  ...
  * **Mac:** requires `GCC 9` / `Clang 9` / `Xcode 11` or above. Note: For `Clang` or` Xcode` compilation, MacOS version 10.15 or above is required.
    ...
    * **GCC/Clang:** o to the corresponding directory in [./projects/Mac](./projects/Mac), and run the `release.sh` / `debug.sh` script.
  * Reference:
